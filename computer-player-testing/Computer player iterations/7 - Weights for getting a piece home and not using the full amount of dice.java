...
public class ComputerPlayer implements PlayerInterface {
	...
	private static final double[] WEIGHT_KNOCKED_PROGRESS = {1, 1.15, 1.5, 2};
	...
	private static final double WEIGHT_PIECES_HOME_NOT_ALL_OUT = 0.5;
	...
	private double scoreTurn(TurnInterface turn, BoardInterface boardBefore, Colour colour, List<Integer> diceValues) {
			...
			if (endLoc <= BoardInterface.NUMBER_OF_LOCATIONS) {
				...
				try {
					if (boardBefore.getBoardLocation(endLoc).numberOfPieces(colour.otherColour()) > 0) {
						...
						if (endLoc > 12) {
							if (endLoc > 18) {
								scoreKnocked += WEIGHT_KNOCKED * WEIGHT_KNOCKED_PROGRESS[3];
							} else {
								scoreKnocked += WEIGHT_KNOCKED * WEIGHT_KNOCKED_PROGRESS[2];
							}
						} else if (endLoc > 6){
							scoreKnocked += WEIGHT_KNOCKED * WEIGHT_KNOCKED_PROGRESS[1];
						} else {
							scoreKnocked += WEIGHT_KNOCKED * WEIGHT_KNOCKED_PROGRESS[0];
						}
					}
				} catch (NoSuchLocationException e) {
					// This should never be reached
		...
		double scoreAlone = (double) (piecesBefore[1] - piecesAfter[1]) * WEIGHT_LONE_PIECES;
		double scoreDoubles = (double) (piecesAfter[2] - piecesAfter[2]);
		scoreDoubles *= WEIGHT_DOUBLE_CREATION;
