 ...
 * Testing showed the player performed 500+ turns per second (including processing of the game) on a stock 4790k
 */
public class ComputerPlayer implements PlayerInterface {
	...
	private static final double[] WEIGHT_DICE_USE_THRESHOLDS = {1, 2, 3};
	private static final double[][] WEIGHT_DICE_USE_THRESHOLDS_MULT = {{0, 0.2}, {1, 0.5}, {2, 1}}; // Element [i][0] = ith threshold, all pieces out
	private static final double[] WEIGHT_BLOCKING_START = { 0.1, 0.25, 0.5, 1 };
	private static final double WEIGHT_BLOCKING_START_OPP_BEHIND_MULT = 2;
	...
	public TurnInterface getTurn(Colour colour, BoardInterface board, List<Integer> diceValues) throws PauseException {
		...
		// Attempt to update the GUI (if it exists then an exception will occur)
		try {
	...
	private double scoreTurn(TurnInterface turn) {
				...
				try {
					if (boardBefore.getBoardLocation(endLoc).numberOfPieces(colour.otherColour()) > 0) {
						...
						for (int i = 18; i >= 0; i -= 6) {
							if (endLoc > i) {
								scoreKnocked += WEIGHT_KNOCKED * WEIGHT_KNOCKED_PROGRESS_MULT[i / 6];
							}
						}
					}
				} catch (NoSuchLocationException e) {
		...
		if (diceNotUsed > 0) {
			...
			int diceThreshold = 0;
			int arraySize = WEIGHT_DICE_USE_THRESHOLDS.length - 1;
			while (diceThreshold < arraySize && WEIGHT_DICE_USE_THRESHOLDS[diceThreshold + 1] <= diceNotUsed) {
				diceThreshold++;
			}
